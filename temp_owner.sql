-- =====================================================================
-- VUES DASHBOARD FRONT OFFICE - PROPRIÃ‰TAIRE DE PARKINGS
-- =====================================================================

-- Vue 1: Revenus mensuels du propriÃ©taire avec Ã©volution
...
-- Vue 2: Statistiques des rÃ©servations du propriÃ©taire
CREATE OR REPLACE VIEW public.v_owner_reservations_stats AS
SELECT 
    p.id_users AS owner_id,
    
    -- RÃ©servations mois courant
    COUNT(CASE 
        WHEN DATE_TRUNC('month', r.creation_date) = DATE_TRUNC('month', CURRENT_DATE)
        THEN r.id_reservation 
    END) AS reservations_mois_courant,
    
    -- RÃ©servations mois prÃ©cÃ©dent
    COUNT(CASE 
        WHEN DATE_TRUNC('month', r.creation_date) = DATE_TRUNC('month', CURRENT_DATE - INTERVAL '1 month')
        THEN r.id_reservation 
...
-- Vue 3: DisponibilitÃ© des parkings du propriÃ©taire
...
SELECT 
    ...
    -- Places occupÃ©es actuellement (rÃ©servations en cours)
    COALESCE(SUM(CASE 
        WHEN r.start_datetime <= NOW() 
        AND r.end_datetime >= NOW() 
        AND rs.value_ = 2  -- ConfirmÃ©es
        THEN rv.numbers 
        ELSE 0 
...
-- Vue 4: Revenus hebdomadaires du propriÃ©taire (7 derniers jours)
...
-- Vue 5: Notifications rÃ©centes du propriÃ©taire (filtrÃ©e cÃ´tÃ© app avec WHERE id_users = ?)
...
-- Vue 6: Dashboard synthÃ¨se propriÃ©taire (toutes mÃ©triques en une requÃªte)
